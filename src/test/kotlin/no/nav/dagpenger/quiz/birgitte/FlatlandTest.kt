...
internal class FlatlandTest {
    ...
    @Test
    fun `skal varsle om uløste behov`() =
        runBlocking {
            val rapid: TestRapid =
                TestRapid().apply {
            ...
            rapid.sendTestMessage(
                uLøsteBehovJson(opprettet = LocalDateTime.now().minusHours(1)),
            ) // Manipulate time to trigger 30 minute threshold
            delay(100)
    ...
    @Test
    fun `skal ikke varsle om løste behov`() =
        runBlocking {
            val rapid: TestRapid =
                TestRapid().apply {
                    Flatland(rapidsConnection = this, delay = delay)
    ...
    fun `Ingen løste behov`() =
        runBlocking {
            ...
            rapid.sendTestMessage(
                ingenLøsteBehov(opprettet = LocalDateTime.now().minusHours(1)),
            ) // Manipulate time to trigger 30 minute threshold
            delay(1000)
            with(rapid.inspektør) {
                assertEquals(1, size)
